...
const stringify = (ast) => {
  return ast.reduce((result, node) => {
    if (node.type === "literal") {
      ...
      return result + stringifyExpression(node);
    }
  }, "");
};

const stringifyExpression = (expression) => {
  const variableList = expression.variables.map((variable) => {
    let variableSpecifier = variable.name;
    if (variable.maxLength !== undefined) {
      variableSpecifier += `:${variable.maxLength}`;
  ...
  return `{${expression.type}${variableList}}`;
};
...
module.exports = { parse, expand, stringify };
